...
PATCH="if [[ -f $FILE ]]; then if egrep -q '^SUBSYSTEM==\"memory' /lib/udev/rules.d/40-vm-hotadd.rules; then sed -i -e 's/SUBSYSTEM==\"memory/# SUBSYSTEM==\"memory/' /lib/udev/rules.d/40-vm-hotadd.rules; fi; fi"
